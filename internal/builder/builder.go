...
func (b *Builder) Build() (*BuildReport, error) {
	...
	if !b.DryRun {
		...
		if err := os.WriteFile(filepath.Join(stateDir, "toolkit-version.json"), []byte(versionState), 0644); err != nil {
			return nil, fmt.Errorf("saving toolkit version: %w", err)
		}
	}
...
func renderToolkitOverview(c *constitution.Constitution, plan *resolution.RolloutPlan) string {
	...
	fmt.Fprintf(&buf, "- Safety mode: %s\n", c.Quality.SafetyMode)
	fmt.Fprintf(&buf, "- Blocking checks: %v\n", c.Quality.BlockOn)
	fmt.Fprintf(&buf, "- Safe autofix: %v\n", c.Automation.AllowSafeAutofix)
