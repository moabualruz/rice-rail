...
func TestScanGoFixture(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(dir, ".golangci.yml"), []byte("linters:\n  enable:\n    - errcheck"), 0644)
	_ = os.WriteFile(filepath.Join(dir, "Makefile"), []byte("build:\n\tgo build ./..."), 0644)
	_ = os.WriteFile(filepath.Join(dir, "main.go"), []byte("package main\nfunc main() {}"), 0644)

	_ = os.MkdirAll(filepath.Join(dir, "cmd", "app"), 0755)
	_ = os.WriteFile(filepath.Join(dir, "cmd", "app", "main.go"), []byte("package main"), 0644)

	_ = os.MkdirAll(filepath.Join(dir, "internal", "core"), 0755)
	_ = os.WriteFile(filepath.Join(dir, "internal", "core", "core.go"), []byte("package core"), 0644)
...
func TestScanTSMonorepo(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(dir, "package.json"), []byte(pkgJSON), 0644)
	_ = os.WriteFile(filepath.Join(dir, "turbo.json"), []byte(`{"pipeline": {}}`), 0644)
	_ = os.WriteFile(filepath.Join(dir, "tsconfig.json"), []byte(`{"compilerOptions": {}}`), 0644)
	_ = os.WriteFile(filepath.Join(dir, "biome.json"), []byte(`{"linter": {}}`), 0644)

	_ = os.MkdirAll(filepath.Join(dir, "packages", "a"), 0755)
	_ = os.WriteFile(filepath.Join(dir, "packages", "a", "package.json"), []byte(`{"name": "a"}`), 0644)
	_ = os.WriteFile(filepath.Join(dir, "packages", "a", "index.ts"), []byte("export const a = 1;"), 0644)

	_ = os.MkdirAll(filepath.Join(dir, "packages", "b"), 0755)
	_ = os.WriteFile(filepath.Join(dir, "packages", "b", "package.json"), []byte(`{"name": "b"}`), 0644)
	_ = os.WriteFile(filepath.Join(dir, "packages", "b", "index.ts"), []byte("export const b = 2;"), 0644)
...
func TestScanPythonProject(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(dir, "ruff.toml"), []byte("[lint]\nselect = [\"E\", \"F\"]"), 0644)
	_ = os.WriteFile(filepath.Join(dir, "conftest.py"), []byte("import pytest"), 0644)

	_ = os.MkdirAll(filepath.Join(dir, "src"), 0755)
	_ = os.WriteFile(filepath.Join(dir, "src", "app.py"), []byte("def main(): pass"), 0644)
...
func TestScanRustProject(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(dir, "clippy.toml"), []byte("cognitive-complexity-threshold = 30"), 0644)

	_ = os.MkdirAll(filepath.Join(dir, "src"), 0755)
	_ = os.WriteFile(filepath.Join(dir, "src", "main.rs"), []byte("fn main() {}"), 0644)
...
func TestScanMultiLanguage(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(dir, "index.ts"), []byte("export default {}"), 0644)
